...
1. Read [`CLAUDE.md`](../CLAUDE.md) — it carries the canonical-source
   table, language policy, accessibility rules, the metaproject
   plumbing, and the agent workflow.
...
`graphify-out-meta/graph.json` for cross-project questions,
`<sibling>/graphify-out/graph.json` for per-sibling questions) over
raw source browsing — this is already documented in `CLAUDE.md`
§"graphify". For metaproject plumbing, the script
`scripts/sync-graphify-skill.js` is the only sanctioned cross-project
mutation tool (read `CLAUDE.md` §"Commands" for the full surface).
