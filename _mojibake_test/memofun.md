...
| Project roles (user, support, build) and who reads what first (Memofun: persona usuaria / apoyo / construcci�n) | [`doc/en/roles.md`](doc/en/roles.md) � [`doc/es/roles.md`](doc/es/roles.md) |
...
- **1.4.6 Contrast (Enhanced)**  text e 7:1 (large text e 4.5:1).
...
1. Read the content config: either a `doc/curriculum/**/*.md` file
   ...
   `# �ndice` bullet list  see `doc/es/tecnico.md` �3 and �8 for
...
3. Write `cantidad` cards in Spanish (or `idioma` if set), each
   ...
   - **Format: clue first, concept as the answer.** `pregunta`
     ...
     learner to name or identify it ("�C�mo se llama...?",
     "�Qui�n...?", "�Qu� n�mero es?"). `respuesta` is ONLY the
   ...
   - Control conceptual density, not just sentence length  Easy
     ...
     (e.g. describe *desenga�o* as "a soap bubble: it shines,
   ...
   - When it fits naturally, weave ONE surprising/curious fact (a
     "�Sab�as que...?") into the clue in `pregunta`  never forced,
     never at the cost of clarity, and never in `respuesta`.
   - `pregunta`: 2-4 clue sentences d12 words each, one idea per
   ...
   - If there's a `# �ndice`: spread the cards across **every**
...
9. **Per-card image  always a thumbnail, on every card by
   ...
   200 KB on disk** (d1024 px on the long edge) 
